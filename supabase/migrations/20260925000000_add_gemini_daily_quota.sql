...
revoke all on schema planly_private from public, anon, authenticated;

create table if not exists planly_private.gemini_daily_usage (
  user_id uuid not null references auth.users(id) on delete cascade,
  usage_date date not null,
  request_count integer not null default 0,
  updated_at timestamptz not null default now(),
  primary key (user_id, usage_date),
  constraint gemini_daily_usage_request_count_check
    check (request_count between 0 and 50)
);

alter table planly_private.gemini_daily_usage enable row level security;
