...
- **Thiết kế theo phân tầng chuẩn**:
  - **Tầng Domain & Thuật toán (`src/services/ai/`)**: Độc lập với UI, viết bằng pure TypeScript và có unit test cho các luồng cốt lõi.
    ...
    - `aiProvider.ts`: `PlanlyAiProvider` triển khai mẫu thiết kế Provider linh hoạt: gọi Google Gemini qua Supabase Edge Function đã xác thực, giới hạn 50 lượt cloud mỗi tài khoản/ngày và tự động fallback sang `nlpParser` offline khi cloud không khả dụng.
